<template>
  ...
  >
    ...
    >
      <IndicatorIcon :class="getDealStatus(s.status)?.color" />
      {{ s.status }}
  ...
  <DealsListView
    ...
    :status-filter-options="orderedStatusCounts"
    :active-status-filter="activeStatus"
    :options="{
      showTooltip: false,
      resizeColumn: true,
    ...
    @statusFilter="(s) => setStatusFilter(s)"
    @loadMore="() => loadMore++"
...
import { Tooltip, Avatar, Dropdown, createResource, FeatherIcon, toast } from 'frappe-ui'
...
function shareView() {
  const filters = deals.value?.params?.filters || {}
...
const orderedStatusCounts = computed(() =>
  [...(statusCounts.data || [])]
    .map((s) => ({ ...s, color: getDealStatus(s.status)?.color }))
    .sort(
      (a, b) =>
